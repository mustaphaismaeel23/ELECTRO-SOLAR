...
def init_db():
    ...
    add_column_if_missing('products', 'wholesale_price', 'wholesale_price REAL NOT NULL DEFAULT 0')
    add_column_if_missing('customers', 'customer_type', "customer_type TEXT NOT NULL DEFAULT 'retail'")
    add_column_if_missing('sales', 'sales_type', "sales_type TEXT NOT NULL DEFAULT 'retail'")
    ...
    if not cur.fetchone():
        ...
        prods = [
            ('150W Mono Solar Panel','SOL-001',cid('Solar Panels'),bid('Felicity Solar'),'FP-150M',150,18,None,None,'1 year',45000,58000,50000,12,5,'piece','150W monocrystalline solar panel'),
            ('300W Poly Solar Panel','SOL-002',cid('Solar Panels'),bid('Prag'),'PRG-300P',300,24,None,None,'1 year',82000,105000,92000,8,3,'piece','300W polycrystalline solar panel'),
            ('100Ah AGM Battery','BAT-001',cid('Batteries'),bid('Luminous'),'LUM-100AGM',None,12,None,'100Ah','18 months',38000,50000,44000,5,4,'piece','100Ah 12V sealed AGM deep cycle battery'),
            ('200Ah Tubular Battery','BAT-002',cid('Batteries'),bid('Sukam'),'SK-200T',None,12,None,'200Ah','2 years',65000,82000,74000,3,4,'piece','200Ah 12V flooded tubular battery'),
            ('1.5kVA Pure Sine Inverter','INV-001',cid('Inverters'),bid('UTL'),'UTL-1500',1500,24,None,'1.5kVA','1 year',55000,72000,64000,6,3,'piece','1.5kVA 24V pure sine wave inverter'),
            ('3.5kVA Hybrid Inverter','INV-002',cid('Inverters'),bid('Luminous'),'LUM-3500H',3500,48,None,'3.5kVA','2 years',95000,125000,110000,4,2,'piece','3.5kVA 48V hybrid solar inverter'),
            ('30A MPPT Controller','CHG-001',cid('Charge Controllers'),bid('Generic'),'MPPT-30A',None,12,30,None,'1 year',12000,17500,15000,10,5,'piece','30A MPPT solar charge controller'),
            ('40A PWM Controller','CHG-002',cid('Charge Controllers'),bid('Generic'),'PWM-40A',None,12,40,None,'1 year',8500,12000,10500,15,5,'piece','40A PWM solar charge controller'),
            ('9W LED Bulb','BLB-001',cid('Bulbs & Lighting'),bid('Nexus'),'NX-9W',9,220,None,None,'6 months',850,1400,1150,2,10,'piece','9W E27 LED bulb warm white'),
            ('4mm² Solar Cable','WIR-001',cid('Wires & Cables'),bid('Generic'),'SC-4MM',None,None,None,None,None,380,600,520,200,50,'metre','4mm² DC solar cable per metre'),
            ('6mm² Solar Cable','WIR-002',cid('Wires & Cables'),bid('Generic'),'SC-6MM',None,None,None,None,None,520,850,730,150,50,'metre','6mm² DC solar cable per metre'),
            ('32A Single Pole MCB','MCB-001',cid('Circuit Breakers & Fuses'),bid('Generic'),'MCB-32SP',None,230,32,None,'1 year',1800,2800,2400,20,10,'piece','32A single pole MCB'),
            ('PVC 3-Pin Socket','SOC-001',cid('Sockets & Switches'),bid('Generic'),'PVC-3P',None,230,13,None,'6 months',450,800,650,30,10,'piece','13A PVC 3-pin wall socket'),
            ('MC4 Connector Pair','CON-001',cid('Connectors'),bid('Generic'),'MC4-PAIR',None,None,None,None,None,350,600,500,50,20,'pair','MC4 solar connector pair M+F'),
        ]
        for p in prods:
            db.execute("""INSERT INTO products
                (name,sku,category_id,brand_id,model_number,wattage,voltage,amperage,capacity,warranty,
                 cost_price,selling_price,wholesale_price,quantity,reorder_level,unit,description)
                VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)""", p)

    # Seed customers
    cur = db.execute("SELECT id FROM customers WHERE phone='08011111111'")
    if not cur.fetchone():
        db.execute("INSERT INTO customers (name,customer_type,phone,address,credit_limit) VALUES (?,?,?,?,?)",
                   ('Emeka Solar Ltd','wholesale','08011111111','15 Industrial Road, Lagos',1000000))
        db.execute("INSERT INTO customers (name,customer_type,phone,address,credit_limit,credit_balance) VALUES (?,?,?,?,?,?)",
                   ('Bright Electrical','wholesale','08022222222','42 Market Street, Abuja',500000,45000))
        db.execute("INSERT INTO customers (name,customer_type,phone,address,credit_limit) VALUES (?,?,?,?,?)",
                   ('Green Energy Co.','wholesale','08033333333','7 Solar Avenue, Enugu',750000))
...
def products():
    ...
    conn = get_db()
    items = conn.execute("""
        SELECT p.id, p.name, p.sku, p.category_id, p.brand_id, p.model_number,
               p.wattage, p.voltage, p.amperage, p.capacity, p.warranty,
               p.cost_price, p.selling_price, p.wholesale_price,
               p.quantity, p.reorder_level, p.unit, p.description,
    ...
    try:
        if pid:
            execute("""UPDATE products SET name=?,sku=?,category_id=?,brand_id=?,model_number=?,
                ...
                wholesale_price=?,reorder_level=?,unit=?,description=?,updated_at=datetime('now') WHERE id=?""",
                (f['name'],f['sku'],ni('category_id'),ni('brand_id'),nv('model_number'),
                 nf('wattage'),nf('voltage'),nf('amperage'),nv('capacity'),nv('warranty'),
                 float(f['cost_price']),float(f['selling_price']),float(f.get('wholesale_price',0)),
                 int(f.get('reorder_level',5)),f.get('unit','piece'),nv('description'),pid))
            ...
            cur = execute("""INSERT INTO products
                ...
                (f['name'],f['sku'],ni('category_id'),ni('brand_id'),nv('model_number'),
                 nf('wattage'),nf('voltage'),nf('amperage'),nv('capacity'),nv('warranty'),
                 float(f['cost_price']),float(f['selling_price']),float(f.get('wholesale_price',0)),qty,
...
def pos_complete():
    ...
    try:
        ...
        cur = db.execute("""INSERT INTO sales
            (invoice_no,customer_id,user_id,sales_type,subtotal,discount,total,amount_paid,balance,payment_method,status,notes)
            VALUES (?,?,?,?,?,?,?,?,?,?,?,?)""",
            (data['invoice_no'], data.get('customer_id') or None, session['user_id'],
             data.get('sales_type','retail'),
...
def sales():
    ...
    if stype:
        items = query("""SELECT s.*, COALESCE(c.name,'Walk-in') customer_name, u.full_name staff_name
            ...
            WHERE date(s.created_at) BETWEEN ? AND ? AND s.sales_type=? ORDER BY s.created_at DESC""", (from_date, to_date, stype))
    else:
        items = query("""SELECT s.*, COALESCE(c.name,'Walk-in') customer_name, u.full_name staff_name
            FROM sales s LEFT JOIN customers c ON s.customer_id=c.id JOIN users u ON s.user_id=u.id
    ...
    return render_template('sales.html', sales=items, totals=totals, from_date=from_date, to_date=to_date, sales_type=stype)
...
def customers_save():
    f = request.form
    if f.get('id'):
        execute("UPDATE customers SET name=?,customer_type=?,phone=?,email=?,address=?,credit_limit=? WHERE id=?",
                (f['name'],f.get('customer_type','retail'),f.get('phone',''),f.get('email',''),f.get('address',''),float(f.get('credit_limit',0)),f['id']))
    else:
        execute("INSERT INTO customers (name,customer_type,phone,email,address,credit_limit) VALUES (?,?,?,?,?,?)",
                (f['name'],f.get('customer_type','retail'),f.get('phone',''),f.get('email',''),f.get('address',''),float(f.get('credit_limit',0))))
...
def reports():
    ...
    types = [
        ...
        ('wholesale_vs_retail', 'Wholesale vs Retail'),
        ('wholesale_summary', 'Wholesale Sales'),
        ('retail_summary', 'Retail Sales'),
    ...
    if rtype == 'sales':
    ...
    elif rtype == 'wholesale_vs_retail':
        data = query("""SELECT sales_type, COUNT(*) transactions,
            COALESCE(SUM(total),0) revenue, COALESCE(SUM(discount),0) discounts,
            COALESCE(SUM(balance),0) outstanding
            FROM sales WHERE date(created_at) BETWEEN ? AND ?
            GROUP BY sales_type ORDER BY sales_type""", (from_date, to_date))
    elif rtype == 'wholesale_summary':
        data = query("""SELECT date(created_at) day, COUNT(*) transactions,
            COALESCE(SUM(total),0) revenue, COALESCE(SUM(discount),0) discounts
            FROM sales WHERE sales_type='wholesale' AND date(created_at) BETWEEN ? AND ?
            GROUP BY date(created_at) ORDER BY day""", (from_date, to_date))
    elif rtype == 'retail_summary':
        data = query("""SELECT date(created_at) day, COUNT(*) transactions,
            COALESCE(SUM(total),0) revenue, COALESCE(SUM(discount),0) discounts
            FROM sales WHERE sales_type='retail' AND date(created_at) BETWEEN ? AND ?
            GROUP BY date(created_at) ORDER BY day""", (from_date, to_date))
    elif rtype == 'daily_detail':
        data = query("""SELECT s.id, s.invoice_no, s.sales_type, COALESCE(c.name,'Walk-in') customer_name,
        ...
        data = query("""SELECT c.id, c.name, c.customer_type, c.phone, c.email, c.credit_limit, c.credit_balance,
...
@login_required
def api_product(pid):
    p = query("SELECT id,name,selling_price,wholesale_price,cost_price,quantity,unit FROM products WHERE id=?", (pid,), one=True)
